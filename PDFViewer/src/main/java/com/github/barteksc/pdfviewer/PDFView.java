...
import com.github.barteksc.pdfviewer.listener.OnCopySelectedListener;
...
import com.github.barteksc.pdfviewer.listener.OnSearchListener;
import com.github.barteksc.pdfviewer.listener.OnSelectListener;
import com.github.barteksc.pdfviewer.listener.OnTapListener;
import com.github.barteksc.pdfviewer.listener.OnZoomAnimationUpdate;
...
public class PDFView extends RelativeLayout {
    ...
    public int typeTool = -1;
    public String copyLanguage;
    public String copyMessage;
    public int currentPageSearch = -1;
    ...
    public void nextPage() {
        List<Object> listPageSearch = Arrays.asList(searchRecords.keySet().toArray());
        if (listPageSearch.contains(currentPageSearch)) {
            ArrayList<SearchRecordItem> listResult = new ArrayList<>(Objects.requireNonNull(searchRecords.get(currentPageSearch)).data);
            if (listResult.size() > 0) {
                ...
                redrawSel();
                if (currentPageSearch != currentPage) {
                    jumpTo(currentPageSearch);
                }
                if (indexResult >= listResult.size() && currentPageSearch == (Integer) listPageSearch.get(listPageSearch.size() - 1)) {
                    currentPageSearch = (Integer) listPageSearch.get(0);
                    jumpTo(currentPageSearch);
                    indexResult = -1;
                } else {
                    if (indexResult >= listResult.size()) {
                        for (int index = 0; index < listPageSearch.size(); index++) {
                            if (currentPageSearch < (Integer) listPageSearch.get(index)) {
                                currentPageSearch = (Integer) listPageSearch.get(index);
                                jumpTo(currentPageSearch);
                                indexResult = -1;
                                break;
                            }
    ...
    public void prePage() {
        ...
        if (listPageSearch.contains(currentPageSearch)) {
            ...
            if (listResult.size() > 0) {
                ...
                if (indexResult < 0  && currentPageSearch == (Integer) listPageSearch.get(0)) {
                    currentPageSearch = (Integer) listPageSearch.get(listPageSearch.size() - 1);
                    jumpTo(currentPageSearch);
                    indexResult = -1;
                } else {
                    if (indexResult < 0) {
                        for (int index = listPageSearch.size() - 1; index >= 0; index--) {
                            if (currentPageSearch > (Integer) listPageSearch.get(index)) {
                                currentPageSearch = (Integer) listPageSearch.get(index);
                                jumpTo(currentPageSearch);
                                indexResult = -1;
    ...
    public void setSelectionPaintView(PDocSelection sv, String copyLanguage, String copyMessage) {
    ...
    public void endSearch(ArrayList<SearchRecord> arr) {
        Log.e("TAG", "endSearch: ");
        selectionPaintView.invalidate();
        callbacks.callOnSearchListener();
...
*/
   ...
   /* public ArrayList<RectF> mergeLineRects(List<RectF> selRects, RectF box) {
    ...
    public void moveTo(float offsetX, float offsetY) {
        ...
        callbacks.callOnZoomAnimationUpdate();
    }
    ...
    public class Configurator {
        ...
        private OnCopySelectedListener onCopySelectedListener;

        private OnZoomAnimationUpdate onZoomAnimationUpdate;
        ...
        public Configurator onSearchComplete(OnSearchListener onSearchListener) {
            this.onSearchListener = onSearchListener;
            return this;
        }

        public Configurator onSelection(OnSelectListener onSelectListener) {
            this.onSelectListener = onSelectListener;
            return this;
        }

        public Configurator onCopySelected(OnCopySelectedListener onCopySelectedListener) {
            this.onCopySelectedListener = onCopySelectedListener;
            return this;
        }

        public Configurator onZoomAnimationUpdate(OnZoomAnimationUpdate onZoomAnimationUpdate) {
            this.onZoomAnimationUpdate = onZoomAnimationUpdate;
            return this;
        }
        ...
        public void load() {
            ...
            PDFView.this.callbacks.setOnSearchListener(onSearchListener);
            PDFView.this.callbacks.setOnSelectListener(onSelectListener);
            PDFView.this.callbacks.setOnCopySelectedListener(onCopySelectedListener);
            PDFView.this.callbacks.setOnZoomAnimationUpdate(onZoomAnimationUpdate);
