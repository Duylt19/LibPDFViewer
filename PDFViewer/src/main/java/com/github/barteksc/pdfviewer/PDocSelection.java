...
public class PDocSelection extends View {
    ...
    public boolean isShowItemCopy = false;
    RectF clipboardRect = new RectF();
    float leftClipboard, topClipboard;
    Paint drawingPaintClipboard = new Paint();
    ...
    private void init() {
        ...
        drawingPaintClipboard.setColor(Color.BLACK);
        drawingPaintClipboard.setStyle(Paint.Style.FILL);
    }
    ...
    public ArrayList<ArrayList<RectF>> rectPool = new ArrayList<>();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (pDocView.isSearching) {
            ...
            for (SearchRecord record : searchRecordList) {
                if (record != null) {
                    ...
                    ArrayList<SearchRecordItem> data = record.data;
                    for (int j = 0, len = data.size(); j < len; j++) {
                        ...
                        if (rects != null) {
                            for (RectF rI : rects) {
                                ...
                                VR.set(-2, -3, bmWidth + 2, bmHeight + 3);
                                if (page == pDocView.currentPageSearch && j == pDocView.indexResult){
                                    rectHighlightPaint.setColor(getResources().getColor(R.color.heightlight_color_current));
                                }else {
                                    rectHighlightPaint.setColor(getResources().getColor(R.color.heightlight_color));
                                }
        ...
        if (pDocView.hasSelection && pDocView.typeTool != -1 ) {
        ...
        if (isShowItemCopy && pDocView.typeTool == 1 && pDocView.hasSelection) {
            drawingPaintClipboard.setTextSize(50);
            float widthText = drawingPaintClipboard.measureText(pDocView.copyLanguage);
            clipboardRect.set(leftClipboard - ((widthText/2) + 30), topClipboard - 90, leftClipboard + ((widthText/2) + 35), topClipboard);
            canvas.drawRoundRect(clipboardRect, 20, 20, drawingPaintClipboard);
            drawingPaintClipboard.setColor(Color.WHITE);
            canvas.drawText(pDocView.copyLanguage, leftClipboard - (widthText/2 - 5), topClipboard - 30, drawingPaintClipboard);
            drawingPaintClipboard.setColor(Color.BLACK);
        }
    }
